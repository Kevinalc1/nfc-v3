...
export default function EditarProdutoPage() {
  ...
  // Carrega o restaurante do dono logado, as categorias e o prato a editar
  useEffect(() => {
    async function loadContext() {
      ...
      // Garante que o prato pertence a este restaurante (não só ao id)
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#F6F4EF] text-center px-6">
        ...
        <p className="text-sm text-[#8A8375] mb-6">
          Ele pode ter sido excluído ou não pertence ao seu restaurante.
        </p>
        <button
          onClick={() => router.push('/dashboard/produtos')}
          className="bg-[#2B2622] text-[#F6F4EF] text-sm font-medium rounded-sm px-4 py-2.5 hover:bg-[#3F6B4F] transition"
        >
          Voltar para os pratos
        </button>
      </div>
    )
  }
          ...
          <form onSubmit={handleSubmit} className="px-8 py-8 space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-[#8A8375] font-mono">
                  R$
                </span>
                <input
                  id="price"
            ...
            {/* Disponibilidade */}
            <div className="flex items-center justify-between rounded-sm border border-[#DAD5C9] bg-[#FBFAF7] px-4 py-3">
              <div>
                <p className="text-sm font-medium">Disponível no cardápio</p>
                <p className="text-xs text-[#8A8375]">
                  Pausado não aparece para o cliente na mesa.
                </p>
              </div>
              <button
                type="button"
                ...
                className={`relative w-11 h-6 rounded-full transition shrink-0 ${
                  isAvailable ? 'bg-[#3F6B4F]' : 'bg-[#DAD5C9]'
                }`}
              >
                <span
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-[#B8B2A2] mt-1">
                    Escolher um novo arquivo substitui a foto atual. Até{' '}
                    {MAX_IMAGE_MB}MB.
                  </p>
                </div>
              </div>
            ...
            <div>
              ...
              {videoFile ? (
                ...
                <p className="text-[11px] text-[#8A8375] mt-1">
                  Já existe um vídeo salvo para este prato.
                </p>
              ) : null}
              <p className="text-[11px] text-[#B8B2A2] mt-1">
                Escolher um novo arquivo substitui o vídeo atual. Até{' '}
                {MAX_VIDEO_MB}MB.
              </p>
            </div>
